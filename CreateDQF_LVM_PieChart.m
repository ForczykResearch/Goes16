function  [bigreasonval]=CreateDQF_LVM_PieChart(ilevel,titlestr)
% This function will show selected DQF_Overall values for the LVM Metric
% for a single level
% Written By: Stephen Forczyk
% Created: Mar 11,2021
% Revised: Mar 12,2021 did a sort on the rejection values to make the
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine CreateDQF_LVM_PieChart-----');
...
titlestr2=strcat('DQF-LVM-Invalid-Pixels',prefix,suffix);
ht=title(titlestr2);
numvals=length(ph);
cb=jet;
...
eval(cmdString);
plotstr1=strcat('Saved LVM DQF Plot as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
if((iCreatePDFReport==1) && (RptGenPresent==1))
...
close('all');
fprintf(fid,'%s\n','-----Finish plot routine CreateDQF_LVM_PieChart-----');
fprintf(fid,'\n');
end

function  [TT1DSF,sgoodfrac]=DisplayConusTTStabilityIndex(titlestr)
% Display the Total Total Index Stability Data on a CONUS Grid
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine Display CONUS TT Stability Index-----');
eval(['cd ' gridpath(1:length(gridpath)-1)]);
ab=1;
[nrows,ncols]=size(TTS.values);
...
TT1DSF=zeros(numvals2,1);
for k=1:numvals2
    TT1DSF(k,1)=TT1DS(k,1);
end
sgoodfrac=numvals2/(nrows*ncols);
TTmin=TT1DSF(1,1);
TTmax=TT1DSF(numvals2,1);
num99=round(.99*numvals2);
val99=TT1DSF(num99,1);
fracsevere=numhigh/numvals2;
statstr1=strcat('Minimum Value TT=',num2str(TTmin,6),'-Max Value TT=',num2str(TTmax,6),'-fraction severe weather=',num2str(fracsevere,5));
fprintf(fid,'%s\n',statstr1);
...
geoshow(RasterLats,RasterLons,TT,'DisplayType','texturemap');
% zlimits = [-43 56];
% demcmap(zlimits);
demcmap(TT,256,[],cmapland);
hold on
...
tx2=.80;
ty2=.75;
txtstr2='TT-Index-Deg-K';
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',12);
tx3=.13;
ty3=.21;
txtstr3=strcat('The 99 ptile value of the TT index=',num2str(val99,5));
...
eval(cmdString);
plotstr1=strcat('Saved TT Stability Plots as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('TT Index Map'));
    ...
    image.ScaleToFit=0;
    add(chapter,image);
% Now add some text -start by decribing the TT Stability Index Metric
    parastr81=strcat('The Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr82='The Total totals index (TT) is a calculated metric obtained from ABI sensor measurements along with a model atmosphere which provides insight to the level of atmospheric instability.';
    parastr83='Fundamentally,the TT metric is calculated from measures of vertical and cross totals pressure data.';
    parastr84='Values below 45 indicate little probability of thunderstorm activity while values above 55 point to a high probability of thunderstorm activity.';
    parastr85='This metric can be calculated day or night in cloud free areas.Note that the valid range for this stattistic goes from -43 to 56.';
    parastr86=strcat('The fraction of pixels with valid reports showing severe weather is=',num2str(fracsevere,5));
...
fprintf(fid,'%s\n','-----Exit DisplayConus TT Stability Index Data-----');
fprintf(fid,'\n');
end

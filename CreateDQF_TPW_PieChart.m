function  CreateDQF_TPW_PieChart(titlestr)
% This function will show selected DQF_Overall values for the TPW Metric
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine CreateDQF_TPW_PieChart-----');
...
titlestr2=strcat('DQF-TPW-Invalid-Pixels',prefix,suffix);
ht=title(titlestr2);
numvals=length(ph);
cb=jet;
...
eval(cmdString);
plotstr1=strcat('Saved TPW DQF Plot as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
if((iCreatePDFReport==1) && (RptGenPresent==1))
...
close('all');
fprintf(fid,'%s\n','-----Finish plot routine CreateDQF_TPW_PieChart-----');
fprintf(fid,'\n');
end

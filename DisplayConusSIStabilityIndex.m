function  [SI1DSF,sgoodfrac]=DisplayConusSIStabilityIndex(titlestr)
% Display the Showalter Index Stability Data on a CONUS Grid
% The Showalter is similar to the LI index but refers to a diffrent
% pressure level
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine Display CONUS Showalter Stability Index-----');
...
disp(dispstr)
SI=SIS.values;
SI1D=reshape(SI,nrows*ncols,1);
SI1DS=sort(SI1D);
[isevere]=find(SI1DS<=-3);
a1=isempty(isevere);
if(a1==0)
    numsevere=length(isevere);
else
    numsevere=0;
end
[istable]=find(SI1DS>-3);
a2=isempty(istable);
if(a2==0)
...
nmeas=numsevere + numstable;
fracunstable=numsevere/nmeas;
a1=isnan(SI1DS);
numnan=sum(a1);
numvals2=(nrows*ncols)-(numnan+1);
SI1DSF=zeros(numvals2,1);
for k=1:numvals2
    SI1DSF(k,1)=SI1DS(k,1);
end
sgoodfrac=numvals2/(nrows*ncols);
SImin=SI1DSF(1,1);
SImax=SI1DSF(numvals2,1);
num99=round(.99*numvals2);
val99=SI1DSF(num99,1);
zlow=floor(SImin);
zhigh=ceil(SImax);
statstr1=strcat('Minimum Value SI=',num2str(SImin,6),'-Max Value SI=',num2str(SImax,6));
fprintf(fid,'%s\n',statstr1);
statstr2=strcat('Num of unstable pixels=',num2str(numsevere),'-numstable=',num2str(numstable),'-frac unstable=',num2str(fracunstable,6));
fprintf(fid,'%s\n',statstr2);
...
geoshow(RasterLats,RasterLons,SI,'DisplayType','texturemap');
zlimits = [zlow zhigh];
demcmap(zlimits);
...
tx2=.80;
ty2=.75;
txtstr2='SI-Index-Deg-C';
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',12);
tx3=.13;
ty3=.21;
txtstr3=strcat('The 99 ptile value of the SI index=',num2str(val99,5));
...
eval(cmdString);
plotstr1=strcat('Saved SI Stability Plots as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('Showalter Index Map'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Showalter Stability Index For File-',GOESShortFileName);
    ...
    image.ScaleToFit=0;
    add(chapter,image);
% Now add some text -start by decribing the Showalter Stability Index Metric
    parastr111=strcat('The Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr112='As shown above the Showalter (SI) index is quite similar to the Lift Index (LI). The difference is that the reference pressure level of 850 kPa instead of 100 kPa.';
    parastr113='Basically this index measures the difference in temperature between the current air parcel and the equilibrium temperature at this level.';
    parastr114='An SI values less than -3 indicates a potential for severe weather.';
    parastr115='This metric can be calculated day or night in cloud free areas.';
    parastr116=strcat('The fraction of unstable pixels is=',num2str(fracunstable,5),'.');
    parastr119=strcat(parastr111,parastr112,parastr113,parastr114,parastr115,parastr116);
    p11 = Paragraph(parastr119);
    p11.Style = {OuterMargin("0pt", "0pt","10pt","10pt")};
    add(chapter,p11);
    close('all');
%    add(rpt,chapter);
end
fprintf(fid,'%s\n','-----Exit DisplayConus Showalter Stability Index Data-----');
fprintf(fid,'\n');
end

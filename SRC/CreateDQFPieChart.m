function  CreateDQFPieChart(titlestr)
% This function will show selecte DQF that cause pixel rejections
% Written By: Stephen Forczyk
% Created: Jan 28,2021
% Revised: Mar 31,2021 corrected area in argument list
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine Create DQFPieChart-----');
...
titlestr2=strcat('DQF-Invalid-Pixels',suffix);
ht=title(titlestr2);
numvals=length(ph);
numvals2=numvals/2;
cb=jet;
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    pdftxtstr=strcat('Cloud Optical Depth DQF Factors-',GOESShortFileName);
...
fprintf(fid,'%s\n','-----Finish plot routine Create DQFPieChart-----');
fprintf(fid,'\n');
end

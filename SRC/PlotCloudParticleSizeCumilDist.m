function PlotCloudParticleSizeCumilDist(OptPS1DSF,sgoodfrac,titlestr)
...
% Created: Feb 01.2021
% Revised: March 31,2021 renamed to PlotCloudParticleSizeCumilDist
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Cloud Particle Particle Size Cumilative Distribution Plot-----');
...
ylabelstr=strcat('Cloud Particle Size-','microns');
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('Cloud Particle Size Cumil Distribution'));
...
% Now add some text 
    ...
    parastr82='This chart shows the cumilative distribution of all the good cloud  particle size estimates.';
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished Cloud Particle Size Cumiliative Distribution Plot-----');
    fprintf(fid,'\n');
end

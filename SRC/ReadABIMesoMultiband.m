function ReadABIMesoMultiband()
% Modified: This function will read in the GOES ABI-L2-Multiband Data
% This is Multiband data (r,g,b) for the Meso1/Meso2 form factord
...
global minCMI meanCMI maxCMI CMI99 CMI95;
global chart_time;
...
fprintf(fid,'\n');
userstr1='******Start Processing Meso Level MultiBand data*****';
fprintf(fid,'%s\n',userstr1);
...
userstr2=strcat('User selected file-',GOESFileName,'-for CONUS true color imagery');
fprintf(fid,'%s\n',userstr2);
...
userstr2='******End Processing CONUS Level MultiBand data*****';
fprintf(fid,'%s\n',userstr2);
...
% use the original file name with a .mat extension
    ...
    dispstr=strcat('User Chose Not To Save Matlab File-',MatFileName);
    disp(dispstr)
    fprintf(fid,'%s\n',dispstr);
end
netcdf.close(ncid);
dispstr='Closing Data file and start making 3 color image';
disp(dispstr)
...
% Display the The True Color RGB data
[iunder]=strfind(GOESFileName,'_');
is=iunder(3)+1;
ie=iunder(4)-1;
fileprefix=GOESFileName(is:ie);
filename=RemoveUnderScores(fileprefix);
titlestr=strcat('TrueColor-CMI-Reflectance-',filename);
DisplayCMIMesoDataRev2(OutputImage,titlestr)
...
fprintf(fid,'%s\n','*****Finished Plotting True Color RGB data*****');
close('all')
end
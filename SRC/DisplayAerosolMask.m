function  DisplayAerosolMask(titlestr)
% Display the aerosol mask from the GOES16/17 data
...
% Created: Feb 8,2021
% Revised: Feb 9,2021 continued work on this routine
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine DisplayAerosolMask-----');
eval(['cd ' gridpath(1:length(gridpath)-1)]);
ab=1;
% Get the AerosolMask Data
Aerosol=AerosolS.values;
[iaero]=find(Aerosol>0);
numaero=length(iaero);
[nrows,ncols]=size(Aerosol);
ntotpixels=nrows*ncols;
aerofrac=numaero/ntotpixels;
...
disp(dispstr)
aerostr1=strcat('Fraction of pixels with aerosol detected=',num2str(aerofrac,6));
fprintf(fid,'%s\n',aerostr1);
aerosolmin=min(min(Aerosol));
aerosolmax=max(max(Aerosol));
aerostr2=strcat('Aerosol mask min value=',num2str(aerosolmin,3),'-mask max value=',num2str(aerosolmax,3));
fprintf(fid,'%s\n',aerostr2);
...
geoshow(RasterLats,RasterLons,Aerosol','DisplayType','texturemap');
demcmap(Aerosol',256,[],cmapland);
hold on
...
tx2=.84;
ty2=.77;
txt2=text(tx2,ty2,'Aerosol Mask','FontWeight','bold','FontSize',12);
tx3=.13;
ty3=.24;
txtstr3=strcat('fraction aerosol pixels=',num2str(aerofrac,6));
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    headingstr1=strcat('Aerosol Mask For-',GOESShortFileName);
    chapter = Chapter("Title",headingstr1);
    add(chapter,Section('Aerosol Mask'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Aerosol Mask For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr2=strcat('This ABI Aerosol Mask is not a directly observed quantity but instead is computed from a number of reflectance measurements made by the ABI sensor.');
    parastr3='This mask is binary so a 0 corresponds to no aerosol contamination present in the pixel to a 1 that signifies an aerosol detection.';
    parastr4='Detection of Aerosol/Smoke or Dust is not an easy measurement and  a zero mask value should be interpreted to mean that the detection threshold was not met.';
    parastr5='The compuatational algorithm to detect the Aerosol works best when there are heavy concentrations of Aerosols over dark pixels and worst for lower concentrations on lighter pixels.';
    parastr6=strcat('In the data sample shown above the fraction of pixels with aerosol detected above threshold was-',num2str(aerofrac,6));
...
close('all');
fprintf(fid,'%s\n','-----Exit plot routine DisplayAerosolMask-----');
fprintf(fid,'\n');
end

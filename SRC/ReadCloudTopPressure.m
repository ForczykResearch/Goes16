...
%        eval([varname '= double(double(netcdf.getVar(ncid,i))*scale + offset);'])
        if(a10==1) 
            ...
            [ilow,jlow]=find(PRES==-1);
            numlow=length(ilow);
            PRES2=PRES;
            [nrows,ncols]=size(PRES);
            PRES3=reshape(PRES,nrows*ncols,1);
            for ik=1:nrows
                for jk=1:ncols
                    value=PRES2(ik,jk);
                    if(value==-1)
                        ...
                        PRES2(ik,jk)=(value+2^15)*double(scale);
                    end
   
                end
            end
            PressureS.values=PRES2;
            CTP=PressureS.values;
            [nrows,ncols]=size(CTP);
            CTP2=reshape(CTP,nrows*ncols,1);
%            hist(CTP2,50);
...
PressureFill=PressureS.FillValue;
% find out how many values are filler
[ifill,jfill]=find(Pressure==PressureFill);
a1=isempty(ifill);
if(a1~=1)
...
dispstr=strcat('Number of Pressure Values=',num2str(numvals),'-Filled Values=',num2str(numfill));
disp(dispstr);
...
ab=1;
% prepare the Data For the CloudTop Pressures
HT=Pressure;

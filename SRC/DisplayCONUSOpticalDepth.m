function  [OptDepth1DSF,sgoodfrac]=DisplayCONUSOpticalDepth(titlestr)
% Display the Optical Cloud Depth on a CONUS Grid
...
% Created: Jan 28,2021
% Revised: Jan 30,3032 made revisions to create pie chart and touch up
% output
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine Display CONUS OpticalDepth-----');
eval(['cd ' gridpath(1:length(gridpath)-1)]);
[nrows,ncols]=size(CODS.values);
...
disp(dispstr)
OptDepth=CODS.values;
OptDepth1D=reshape(OptDepth,nrows*ncols,1);
OptDepth1DS=sort(OptDepth1D);
a1=isnan(OptDepth1DS);
numnan=sum(a1);
numvals2=(nrows*ncols)-(numnan+1);
OptDepth1DSF=zeros(numvals2,1);
for k=1:numvals2
    OptDepth1DSF(k,1)=OptDepth1DS(k,1);
end
...
NumOutlierDayCODPixels=OutlierCODS.values1;
MaxLZARetrievalAngle=QLZAS.value;
MaxSolarRetrievalAngle=DSZAS.value;
minDayCODstr=num2str(CODDayStatS.values1,6);
maxDayCODstr=num2str(CODDayStatS.values2,6);
meanDayCODstr=num2str(CODDayStatS.values3,6);
CODwavestr=num2str(CODWavelengthS,4);
...
if(NumDay>0)
    fracoutlierdaypixels=OutlierCODS.values1/NumDay;
    NumOutlierDay=floor(NumOutlierDayCODPixels);
else
...
fprintf(fid,'%s\n',pixelstr4);
CODstr1=strcat('Min Day COD=',minDayCODstr,'-Mean Day COD=',meanDayCODstr,'-Max Day COD=',maxDayCODstr);
fprintf(fid,'%s\n',CODstr1);
  MiscTable=cell(8,3);
  ...
  MiscTable{4,2}=floor(NumOutlierDayCODPixels);
  MiscTable{4,3}='-';
  ...
  MiscTable{7,1}='Min Daytime Optical Depth';
  MiscTable{7,2}=minDayCODstr;
  MiscTable{7,3}='-';
  MiscTable{8,1}='Mean Daytime Optical Depth';
  MiscTable{8,2}=meanDayCODstr;
  MiscTable{8,3}='-';
  MiscTable{9,1}='Max Daytime Optical Depth';
  MiscTable{9,2}=maxDayCODstr;
  MiscTable{9,3}='-';
  MiscTable{10,1}='Primary Wavelength For COD';
  MiscTable{10,2}=CODwavestr;
...
geoshow(RasterLats,RasterLons,OptDepth,'DisplayType','texturemap');
demcmap(OptDepth,256,[],cmapland);
hold on
...
eval(cmdString);
plotstr1=strcat('Saved Cloud Optical Depth Plots as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    headingstr1=strcat('Cloud Optical Depth For-',GOESShortFileName);
    chapter = Chapter("Title",headingstr1);
    add(chapter,Section('Cloud Optical Depth Map'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Cloud Optical Depth For File-',GOESShortFileName);
...
% Now add some text -start by decribing the COD Metric
    ...
    parastr2='Plotted on the chart is the Cloud Optical Depth (COD) which is a unitless quanity-the bigger the value the more optically dense the cloud.';
    parastr3='COD is calculated by using data measured by band 2 (Vis) and band 6 (near IR),the actual COD metrtic is for 640 nm which is in band 2.';
    parastr4='Calculation of this quantity is best during daylight for cloudy pixels-a solar zenith angle (SZA) less than 65 degrees returns the best results,';
    parastr5='however degraded values can be obtained out to an 82 degree SZA.'; 
    parastr6=strcat(parastr1,parastr2,parastr3,parastr4,parastr5);
    p1 = Paragraph(parastr6);
    p1.Style = {OuterMargin("0pt", "0pt","0pt","10pt")};
    add(chapter,p1);
% Description of cloud optical depth
    parastr41=strcat('A few words are in order to describe what Cloud Optical Depth (COD) is and its utility in weather prediction.',...
        'Basically COD is a unitless measure of cloud optical thickness at .64 microns-typically this depth is nearly flat over the visible optical spectrum.');
...
% add a descriptive paragraph for bt5
      ...
      parastr32='Row 4 shows how many pixels provide out of range estimates of cloud depth.';
      ...
      parastr34='The next 3 rows provide the minimum,mean and maximum values for the COD .';
      parastr35='Finally,the last row provides which was the primary band used to calculate the COD.';
      parastr39=strcat(parastr30,parastr31,parastr32,parastr33,parastr34,parastr35);
...
fprintf(fid,'%s\n','-----Exit plot routine Display CONUS OpticalDepth-----');
fprintf(fid,'\n');
%add(rpt,chapter);
end

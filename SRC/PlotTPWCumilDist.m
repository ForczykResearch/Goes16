function PlotTPWCumilDist(ValidTPW,sgoodfrac,titlestr)
% This routine will plot the cumilative distribution of the TPW
% Estimates
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create TPW Cumilative Distribution Plot-----');
...
numvals=length(ValidTPW);
val01=ValidTPW(round(.01*numvals),1);
val50=ValidTPW(round(.50*numvals),1);
val99=ValidTPW(round(.99*numvals),1);
...
ypi(101)=TPWCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=TPWCumil(ptileind,1);
end
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
ptile75ht=ypi(76,1);
ptile100ht=ypi(101,1);
htstr1=strcat('TPW 25 Ptile=',num2str(ptile25ht,6),'-mm');
htstr2=strcat('TPW 50 Ptile=',num2str(ptile50ht,6),'-mm');
htstr3=strcat('TPW 75 Ptile=',num2str(ptile75ht,6),'-mm');
htstr4=strcat('TPW 100 Ptile=',num2str(ptile100ht,6),'-mm/');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('TPW 25ptile=',num2str(ptile25ht,6),'-TPW 50ptile=',num2str(ptile50ht,6),...
    '-TPW Rate 75ptile=',num2str(ptile75ht,6),'-TPW 100 ptile=',num2str(ptile100ht,6),'-all in mm');
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',8);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('TPWCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('TPW Cumil Distribution'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('TPW Data For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr82='This chart shows the cumilative distribution of all valid TPW estimates.';
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished TPW Cumiliative Distribution Plot-----');
    fprintf(fid,'\n');
end

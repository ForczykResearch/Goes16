function ReadMesoscale1ABIL2CMI()
...
% lonDeg=zeros(numrows,1);
% latDeg=zeros(numcols,1);
% RefVec=zeros(numrows,numcols);
% for i=1:numrows
%     lonDeg(i,1)=XVec(i,1)+westEdge;
%     for j=1:numcols
%         latDeg(j,1)=YVec(j,1)+northEdge;
%     end
% end
% ab=2;
GeoRef = georefcells();
...
% plotm(USALat,USALon)
% GeoRef.RasterExtentInLatitude=[(northEdge-southEdge)/numcols];
% GeoRef.RasterExtentInLongitude=[abs((eastEdge-westEdge))/numrows];
...
titlestr=strcat('CMI-Meso1Data-',filename);
DisplayCMIConusDataRev1(CMI,GeoRef,titlestr)
netcdf.close(ncid);
ab=1;
end
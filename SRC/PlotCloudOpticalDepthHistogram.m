function PlotCloudOpticalDepthHistogram(COD1DSF,sgoodfrac,titlestr)
% This routine will plot a histogram of the Cloud Optical Depth
...
global CODS DQFS tS yS xS tBS goesImagerS yImgS yImgBS;
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Cloud Optical Depth Histogram Plot-----');
...
numpts=length(COD1DSF);
num25=round(.25*numpts);
num50=round(.50*numpts);
num75=round(.75*numpts);
num100=numpts;
val25=COD1DSF(num25,1);
val50=COD1DSF(num50,1);
val75=COD1DSF(num75,1);
val100=COD1DSF(num100,1);
statstr1=strcat('25th ptile COD=',num2str(val25,6),'-50 ptile COD=',num2str(val50,6));
statstr2=strcat('75th ptile COD=',num2str(val75,6),'-100 ptile COD=',num2str(val100,6));
fprintf(fid,'%s\n',statstr1);
fprintf(fid,'%s\n',statstr2);
...
edges=linspace(0,300,301);
h=histogram(COD1DSF,edges,'Normalization','probability','FaceColor','b');
xlabel('Cloud Optical Depth','FontWeight','bold');
...
tx2=.18;
ty2=.06;
txtstr2=strcat('Fraction of Pixels that had good COD-Estimate=',num2str(sgoodfrac,6));
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('Cloud Optical Depth Histogram'));
...
% Now add some text 
    ...
    parastr62='This chart shows this distribution of Cloud Optical Depth as a histogram.';
    parastr63=strcat('The fraction of Pixels that had good COD-Estimate=',num2str(sgoodfrac,6),'.');
    parastr64=strcat('Basic COD distribution data as follows.','The first quartile limit is-',num2str(val25,6),'-and the median value is-',...
        num2str(val50,6),'.');
    ...
    disp(dispstr);
    fprintf(fid,'%s\n','-----Finished Cloud Optical Depth Histogram Plot-----');
    fprintf(fid,'\n');
end

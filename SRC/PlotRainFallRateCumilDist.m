function PlotRainFallRateCumilDist(ValidRainFallRate,sgoodfrac,titlestr)
% This routine will plot the cumilative distribution of the Rainfall Rate
% Estimates
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Rainfall Rate Cumilative Distribution Plot-----');
...
numvals=length(ValidRainFallRate);
val01=ValidRainFallRate(round(.01*numvals),1);
val50=ValidRainFallRate(round(.50*numvals),1);
val99=ValidRainFallRate(round(.99*numvals),1);

RainFallRateCumil=ValidRainFallRate;
% Now create an x array that runs from 0 to 100 p tile
xp=0:1:100;
xp=xp';
ypi=zeros(101,1);
ypi(1)=val01-mod(val01,10);
ypi(101)=RainFallRateCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=RainFallRateCumil(ptileind,1);
end
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
ptile75ht=ypi(76,1);
ptile100ht=ypi(101,1);
htstr1=strcat('RainFall Rate 25 Ptile=',num2str(ptile25ht,6),'-mm/hr');
htstr2=strcat('RainFall Rate 50 Ptile=',num2str(ptile50ht,6),'-mm/hr');
htstr3=strcat('RainFall Rate 75 Ptile=',num2str(ptile75ht,6),'-mm/hr');
htstr4=strcat('RainFall Rate 100 Ptile=',num2str(ptile100ht,6),'-mm/hr');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
ylabelstr=strcat('RainFall Rate-','mm/hr');
...
tx2=.18;
ty2=.06;
txtstr2=strcat('RainFall Rate 25ptile=',num2str(ptile25ht,6),'-RainFall Rate 50ptile=',num2str(ptile50ht,6),...
    '-RainFall Rate 75ptile=',num2str(ptile75ht,6),'-RainFall Rate 100 ptile=',num2str(ptile100ht,6),'-all in mm/hr');
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',8);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('RainFalRateCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('RainFall Rate Cumil Distribution'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('RainFall Rate Data For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr82='This chart shows the cumilative distribution of all valid Rainfall Rate Estimates.';
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished RainFall Rate Cumiliative Distribution Plot-----');
    fprintf(fid,'\n');
end

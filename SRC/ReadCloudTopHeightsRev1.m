function ReadCloudTopHeightsRev1(indx)
...
% Revised From Original Version On Nov 23,2020
% Revision was mean to properly decode 16 bit unsigned data that was scaled
% and used an offset
...
fprintf(fid,'%s\n','------- Start Decode Of Cloud Top Height Data ------');
filestr1=strcat('Opening File-',GOESFileName);
fprintf(fid,'%s\n',filestr1);
    ...
    if flag
        try
            if(a10==1)
               ...
               [HT,numnanvals] = ReplaceFillValues(HT,FillValue);
               [nrows,ncols]=size(HT);
               level=(2^15)-1;
               for ik=1:nrows
                   for jk=1:ncols
...
% use the original file name with a .mat extension
    ...
    disp(dispstr);
% end decode
    fprintf(fid,'%s\n','------- End Decode Of Cloud Top Height Data ------');
    ...
    titlestr=strcat('Cloud-Tops-Histogram-',filename2);
    PlotCloudTopHeightsHistogram(HT,titlestr)
    titlestr=strcat('Cloud-Tops-CumilDist-',filename2);
    PlotCloudTopHeightsCumilDist(HT,titlestr)
end
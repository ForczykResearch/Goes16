function PlotVAHCumilDist(ValidVAH,sgoodfrac,titlestr)
% This routine will plot the cumilative distribution of the VAH metric
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create VAH Cumilative Distribution Plot-----');
...
numvals=length(ValidVAH);
val01=ValidVAH(round(.01*numvals),1);
val50=ValidVAH(round(.50*numvals),1);
val99=ValidVAH(round(.99*numvals),1);
VAHCumil=ValidVAH;
...
ypi(101)=VAHCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=VAHCumil(ptileind,1);
end
ptile05ht=ypi(6,1);
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
...
htstr1=strcat('VAH 25 Ptile=',num2str(ptile25ht,6),'-m');
htstr2=strcat('VAH 50 Ptile=',num2str(ptile50ht,6),'-m');
htstr3=strcat('VAH 75 Ptile=',num2str(ptile75ht,6),'-m');
htstr4=strcat('VAH 100 Ptile=',num2str(ptile100ht,6),'-m');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('VAH 25ptile=',num2str(ptile25ht,6),'-VAH 50ptile=',num2str(ptile50ht,6),...
    '-VAH 75ptile=',num2str(ptile75ht,6),'-VAH 100 ptile=',num2str(ptile100ht,6),'-all in m');
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',10);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('VAHCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('VAH Cumilative Distribution'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('VAH Data For File-',GOESShortFileName);
...
% Now add some text 
    br = PageBreak();
    add(chapter,br);
    parastr101=strcat('The VAH Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr102='This chart shows the cumilative distribution of all valid VAH estimates.';
    parastr103=strcat('The median value for the VAH mesurement is-',num2str(ptile50ht,6),'-meters.');
...
%    add(rpt,chapter);
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished VAH Cumilative Distribution Plot-----');
    fprintf(fid,'\n');
end

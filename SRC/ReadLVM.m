...
% Created: Mar 7,2021
% Revised: Mar 12,2021 code cleanup to get rid of unneeded LOC
% 
% Classification: Unclassified

global MetaDataS LevelSets;
...
LVMS=struct('values',[],'long_name',[],'standard_name',[],'FillValue',[],'Unsigned',[],...
...
PressureS=struct('values',[],'long_name',[],'standard_name',[],...
    'units',[],'axis',[]);
PressureImgS=struct('values',[],'long_name',[],'standard_name',[],...
    'units',[],'axis',[],'bounds',[]);
PressureImgBS=struct('values',[],'long_name',[]);
...
for i = 0:numvars-1
    ...
    for j = 0:numatts - 1
        ...
        a170=strcmp(varname,'pressure');
        a180=strcmp(varname,'pressure_image');
        a190=strcmp(varname,'pressure_image_bounds');
        ...
        if (a10==1)
            ...
            if(a1==1)
                LVMS.long_name=attname2;
            end
            a1=strcmp(attname1,'standard_name');
            if(a1==1)
                LVMS.standard_name=attname2;
            end
       ...
       elseif (a80==1)
        ...
        elseif (a180==1)
            ...
            a1=strcmp(attname1,'long_name');
            if(a1==1)
                PressureImgS.long_name=attname2;
            end
            a1=strcmp(attname1,'standard_name');
            if(a1==1)
                PressureImgS.standard_name=attname2;
            end
            a1=strcmp(attname1,'units');
            if(a1==1)
                PressureImgS.units=attname2;
            end
            a1=strcmp(attname1,'axis');
            if(a1==1)
                PressureImgS.axis=attname2;
            end
            a1=strcmp(attname1,'bounds');
            if(a1==1)
                PressureImgS.bounds=attname2;
            end
        elseif (a190==1)
            attname1 = netcdf.inqAttName(ncid,i,j);
            attname2 = netcdf.getAtt(ncid,i,attname1);
            ...
            a1=strcmp(attname1,'long_name');
            if(a1==1)
                PressureImgBS.long_name=attname2;
            end
        elseif (a240==1)
            attname1 = netcdf.inqAttName(ncid,i,j);
            attname2 = netcdf.getAtt(ncid,i,attname1);
    ...
    if flag
        if(a10==1)
            ...
            FillValue=LVMS.FillValue;
            [LVMV,numnanvals] = ReplaceFillValues3(LVM,FillValue);
            [nlvs,nrows,ncols]=size(LVMV);
            level=(2^15)-1;
            for mk=1:nlvs
                for ik=1:nrows
                    for jk=1:ncols
                        ...
                        if(value<0)
                            ...
                            corval=level+diff;
                            LVMV(mk,ik,jk)=corval;
                        end
                    end
                end
            end
            LVMV2=LVMV*scale+offset;
            LVMS.values=double(LVMV2);
            clear LVMV LVMV2;
        end
        if(a40==1)
        ...
        if(a180==1)
            PressureImgS.values=pressure_image;
        end
        if(a190==1)
            PressureImgBS.values=pressure_image_bounds;
        end
        if(a240==1)
            AlgoS.values='null';
        end
...
if(isavefiles==1)
    ...
    actionstr='save';
    varstr1='LVM MetaDataS  tS yS xS tBS goesImagerS';
    ...
    varstr9=' PressureS PressureImgS PressureImgBS';
    ...
    disp(dispstr);
    savestr1=strcat('User saved LVM data to file-',MatFileName);
    fprintf(fid,'%s\n',savestr1);
else
    savestr2=strcat('User did not save LVM data to file-',MatFileName);
    fprintf(fid,'%s\n',savestr2);
end
...
fprintf(fid,'%s\n',geostr2);
[numlvs,numrows,numcols]=size(LVMS.values);
LevelSets=cell(numlvs,1);
for kk=1:numlvs
   LevelSets{kk,1}=num2str(kk); 
end
GeoRef = georefcells();
...
datastr1=strcat('LVM array dimensions nrows=',num2str(numrows),...
...
igo=1;
clear DisplayCONUS_LVM; % This step is needed to clear persistent variables
while igo>0 % This setup up a loop to processing various file until user decides to STOP RUN
    [indL,tf] = listdlg('PromptString',{'Select one pressure level to plot'},...
    'SelectionMode','single','ListString',LevelSets,'ListSize',[360,300]);
    a1=isempty(indL);
    if(a1==1)
        igo=0;
        clear DisplayCONUS_LVM;
    else
        igo=1;
        nowlevelstr=char(LevelSets{indL,1});
        ilevel=str2num(nowlevelstr);
        titlestr=strcat('LVM-Level-',num2str(ilevel),'-',filename);
        [LVM1DSF,sgoodfrac]=DisplayCONUS_LVM(titlestr,ilevel);
        titlestr=strcat('LVM-Level-',num2str(ilevel),'-Hist-',filename);
        [ValidLVM]=PlotLVMHistogram(LVM1DSF,sgoodfrac,ilevel,titlestr);
        titlestr=strcat('LVM-Level-',num2str(ilevel),'-CumilDist-',filename);
        PlotLVMCumilDist(ValidLVM,sgoodfrac,ilevel,titlestr)
    end
end
add(rpt,chapter);
fprintf(fid,'%s\n','-----Finished LVM Data----');
disp('-----Finished Processing LVM Data-----');
end
function PlotLandSurfaceTempCumilDist(LST1DSF,goodfrac,titlestr)
% This routine will plot the cumilative distribution of the land surface
% temp
% 
% Written By: Stephen Forczyk
% Created: Dec 28,2020
% Revised: Jan 24,2021 added code to include this figure in the PDF report
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Land Surface Temp Cumilative Distribution Plot-----');
...
numvals=length(LST1DSF);
val01=LST1DSF(round(.01*numvals),1);
val50=LST1DSF(round(.50*numvals),1);
val99=LST1DSF(round(.99*numvals),1);
maxLandTemp=max(LST1DSF);
minLandTemp=min(LST1DSF);
%LSTCumil=zeros(numvals,1);
LSTCumil=LST1DSF;
...
ypi(101)=LSTCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=LSTCumil(ptileind,1);
end
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
ptile75ht=ypi(76,1);
ptile100ht=ypi(101,1);
htstr1=strcat('LST 25 Ptile=',num2str(ptile25ht,6),'-Deg K');
htstr2=strcat('LST 50 Ptile=',num2str(ptile50ht,6),'-Deg K');
htstr3=strcat('LST 75 Ptile=',num2str(ptile75ht,6),'-Deg K');
htstr4=strcat('LST 100 Ptile=',num2str(ptile100ht,6),'-Deg K');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
ylabelstr=strcat('Land Surface Temp-','Deg K');
...
tx2=.18;
ty2=.06;
txtstr2=strcat('LST 25ptile=',num2str(ptile25ht,6),'-LST 50ptile=',num2str(ptile50ht,6),...
    '-LST 75ptile=',num2str(ptile75ht,6),'-LST 100 ptile=',num2str(ptile100ht,6));
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',8);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('LSTCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    add(chapter,Section('Land Surface Temp Cumil Distribution'));
...
%     temphighlimit=floor(templowlimit+scalefactor*2^16);
    parastr31=strcat('The Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr32='This chart shows cumilative distribution of all land surface temps.';
...
fprintf(fid,'%s\n',savestr1);
fprintf(fid,'%s\n','-----Finished LST Cumiliative  Distribution Plot-----');
fprintf(fid,'\n');
end

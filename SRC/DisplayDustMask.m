function  DisplayDustMask(titlestr)
% Display the dust mask from the GOES16/17 data
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine DisplayDustMask-----');
eval(['cd ' gridpath(1:length(gridpath)-1)]);
% Get the Dust Mask Data
Dust=DustS.values;
[idust]=find(Dust>0);
numdust=length(idust);
[nrows,ncols]=size(Dust);
ntotpixels=nrows*ncols;
dustfrac=numdust/ntotpixels;
...
disp(dispstr)
duststr1=strcat('Fraction of pixels with smoke detected=',num2str(dustfrac,6));
fprintf(fid,'%s\n',duststr1);
dustmin=min(min(Dust));
dustmax=max(max(Dust));
duststr2=strcat('Dust mask min value=',num2str(dustmin,3),'-mask max value=',num2str(dustmax,3));
fprintf(fid,'%s\n',duststr2);
...
geoshow(RasterLats,RasterLons,Dust','DisplayType','texturemap');
demcmap(Dust',256,[],cmapland);
hold on
...
tx2=.84;
ty2=.77;
txt2=text(tx2,ty2,'Dust Mask','FontWeight','bold','FontSize',12);
tx3=.13;
ty3=.24;
txtstr3=strcat('fraction dust pixels=',num2str(dustfrac,6));
...
%     chapter = Chapter("Title",headingstr1);
    add(chapter,Section('Dust Mask'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Dust Mask For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr2=strcat('This ABI Dust Mask is not a directly observed quantity but instead is computed from a number of reflectance measurements made by the ABI sensor.');
...
close('all');
fprintf(fid,'%s\n','-----Exit plot routine DisplayDustMask-----');
fprintf(fid,'\n');
end

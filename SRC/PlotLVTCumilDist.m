function PlotLVTCumilDist(ValidLVT,sgoodfrac,ilevel,titlestr)
% This routine will plot the cumilative distribution of the LVT metric
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create LVT`Cumilative Distribution Plot-----');
...
numvals=length(ValidLVT);
val01=ValidLVT(round(.01*numvals),1);
val50=ValidLVT(round(.50*numvals),1);
val99=ValidLVT(round(.99*numvals),1);
LVTCumil=ValidLVT;
...
ypi(101)=LVTCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=LVTCumil(ptileind,1);
end
ptile05ht=ypi(6,1);
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
...
htstr1=strcat('LVT 25 Ptile=',num2str(ptile25ht,6),'-Deg-k');
htstr2=strcat('LVT 50 Ptile=',num2str(ptile50ht,6),'-Deg-k');
htstr3=strcat('LVT 75 Ptile=',num2str(ptile75ht,6),'-Deg-k');
htstr4=strcat('LVT 100 Ptile=',num2str(ptile100ht,6),'-Deg-k');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('LVT 25ptile=',num2str(ptile25ht,6),'-LVT 50ptile=',num2str(ptile50ht,6),...
    '-LVT 75ptile=',num2str(ptile75ht,6),'-LVT 100 ptile=',num2str(ptile100ht,6),'-Deg-k');
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',10);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('LVTCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    sectionstr=strcat('LVT-CumilDist-Level-',num2str(ilevel));
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('LVT Data For File-',GOESShortFileName);
...
% Now add some text 
    br = PageBreak();
    add(chapter,br);
    parastr151=strcat('The LVT Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr152='This chart shows the cumilative distribution of all valid LVT estimates.';
    parastr153=strcat('The median value for the LVT mesurement is-',num2str(ptile50ht,6),'-Deg-k.');
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished LVT Cumilative Distribution Plot-----');
    fprintf(fid,'\n');
end

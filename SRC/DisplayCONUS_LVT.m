function  [LVT1DSF,sgoodfrac]=DisplayCONUS_LVT(titlestr,ilevel)
% Display the Legacy Vertical Temperature Profile (LVT) data on a CONUS Grid
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine Display CONUS LVT-----');
eval(['cd ' gridpath(1:length(gridpath)-1)]);
[nlvls,nrows,ncols]=size(LVTS.values);
LVT3D=LVTS.values;
LVT2D=zeros(nrows,ncols);
for i=1:nrows
    for j=1:ncols
        LVT2D(i,j)=LVT3D(ilevel,i,j);
    end
end
LVT2D=LVT2D';
[nrows,ncols]=size(LVT2D);
...
fprintf(fid,'%s\n',zlevelstr1);
LVT1D=reshape(LVT2D,nrows*ncols,1);
LVT1DS=sort(LVT1D);
a1=isnan(LVT1DS);
numnan=sum(a1);
numvals2=(nrows*ncols)-(numnan+1);
LVT1DSF=zeros(numvals2,1);
for k=1:numvals2
    LVT1DSF(k,1)=LVT1DS(k,1);
end
...
geoshow(RasterLats,RasterLons,LVT2D,'DisplayType','texturemap');
demcmap(LVT2D,256,[],cmapland);
hold on
...
eval(cmdString);
plotstr1=strcat('Saved LVT Plots as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    if(ncalls==1)
        headingstr1=strcat('Legacy Vertical Temperature For-',GOESShortFileName);
        chapter = Chapter("Title",headingstr1);
    end
    sectionstr=strcat('LVT-Map-Level-',num2str(ilevel));
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Legacy Vertical Temperature Map-Level=',num2str(ilevel),'-For File-',GOESShortFileName);
...
% Now add some text -start by decribing the LVM Metric
    ...
    parastr2=strcat('Plotted on the chart is the Legacy Vertical Temperature Profile (LVT)-For Level=',num2str(ilevel),'.');
    parastr3='The LVT metric is a GOES legacy product derived from the combination of ABI sensor measurments the Legacy Atmospheric Product(LAP).';
    parastr4='LVT is a data product that has data for 101 pressure layers-for a CONUS level plot the data is in a 500 x 300 element array.';
    ...
    parastr6=strcat('Care should be taken in comparing a pressure level to a physical altitude.',...
...
% Build a Rqmts Table
    if(ncalls==1)
        ...
        RqmtsTable{5,2}='180-320 Deg K';
        RqmtsTable{6,1}='Product Measurement Accuracy';
        RqmtsTable{6,2}='1 K below 400 hPa%';
        ...
        bt2 = BaseTable(tbl2);
        tabletitle = Text('LVT Metric Rqmts');
...
% Description of the Requirements table
        ...
        parastr45='The LVT metric itself should be accurate to 1 deg K when the pressure level is less than 400 hPa or about 7 km.';
        parastr46='Refresh rate for this metric ranges from a little as 5 minutes for meso scale up to 60 for the full disk.';
        parastr47='Data can be taken in day or night,with a latency of 266 sec under clear conditions.';
...
% Now build a DQF Table of key values
        ...
        r.Style = [r.Style {Color('red'),Bold(true)}];
        bt4 = BaseTable(tbl4);
        tabletitle = Text('DQF Overall Table For LVT');
        tabletitle.Bold = false;
...
fprintf(fid,'%s\n','-----Exit plot routine Display CONUS LVT-----');
fprintf(fid,'\n');
end

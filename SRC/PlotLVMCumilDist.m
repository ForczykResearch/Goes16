function PlotLVMCumilDist(ValidLVM,sgoodfrac,ilevel,titlestr)
% This routine will plot the cumilative distribution of the LVM metric
% for a single atmospheric presure level
% Written By: Stephen Forczyk
% Created: March 10,2021
% Revised: March 12,2021 code cleanup
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create LVM`Cumilative Distribution Plot-----');
...
numvals=length(ValidLVM);
val01=ValidLVM(round(.01*numvals),1);
val50=ValidLVM(round(.50*numvals),1);
val99=ValidLVM(round(.99*numvals),1);
LVMCumil=ValidLVM;
...
ypi(101)=LVMCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=LVMCumil(ptileind,1);
end
ptile05ht=ypi(6,1);
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
...
htstr1=strcat('LVM 25 Ptile=',num2str(ptile25ht,6));
htstr2=strcat('LVM 50 Ptile=',num2str(ptile50ht,6));
htstr3=strcat('LVM 75 Ptile=',num2str(ptile75ht,6));
htstr4=strcat('LVM 100 Ptile=',num2str(ptile100ht,6));
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
levelstr1=strcat('Cumilative distribution corresponds to level-',num2str(ilevel),'-the model atmosphere has 101 levels');
fprintf(fid,'%s\n',levelstr1);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('LVM 25ptile=',num2str(ptile25ht,6),'-LVM 50ptile=',num2str(ptile50ht,6),...
    '-LVM 75ptile=',num2str(ptile75ht,6),'-LVM 100 ptile=',num2str(ptile100ht,6));
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',10);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('LVMCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    sectionstr=strcat('LVM-CumilDist-Level-',num2str(ilevel));
    add(chapter,Section(sectionstr));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('LVM Data For File-',GOESShortFileName);
...
% Now add some text 
    br = PageBreak();
    add(chapter,br);
    parastr151=strcat('The LVM Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr152='This chart shows the cumilative distribution of all valid LVM estimates.';
    parastr153=strcat('The median value for the LVM mesurement is-',num2str(ptile50ht,6),'-Relative Humidity.');
    parastr159=strcat(parastr151,parastr152,parastr153);
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished LVM Cumilative Distribution Plot-----');
    fprintf(fid,'\n');
end

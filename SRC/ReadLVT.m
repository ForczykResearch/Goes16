...
LVTS=struct('values',[],'long_name',[],'standard_name',[],'FillValue',[],'Unsigned',[],...
...
for i = 0:numvars-1
    ...
    for j = 0:numatts - 1
        ...
        if (a10==1)
            ...
            if(a1==1)
                LVTS.long_name=attname2;
            end
            a1=strcmp(attname1,'standard_name');
            if(a1==1)
                LVTS.standard_name=attname2;
            end
    ...
    if flag
        if(a10==1)
            ...
            FillValue=LVTS.FillValue;
            [LVTV,numnanvals] = ReplaceFillValues3(LVT,FillValue);
            [nlvs,nrows,ncols]=size(LVTV);
            level=(2^15)-1;
            for mk=1:nlvs
                for ik=1:nrows
                    for jk=1:ncols
                        ...
                        if(value<0)
                            ...
                            corval=level+diff;
                            LVTV(mk,ik,jk)=corval;
                        end
                    end
                end
            end
            LVTV2=LVTV*scale+offset;
            LVTS.values=double(LVTV2);
            minval=min(min(min(LVTV2)));
            maxval=max(max(max(LVTV2)));
            clear LVTV LVTV2;
        end
        if(a40==1)
...
if(isavefiles==1)
    ...
    actionstr='save';
    varstr1='LVT MetaDataS  tS yS xS tBS goesImagerS';
    ...
    disp(dispstr);
    savestr1=strcat('User saved LVT data to file-',MatFileName);
    fprintf(fid,'%s\n',savestr1);
else
    savestr2=strcat('User did not save LVT data to file-',MatFileName);
    fprintf(fid,'%s\n',savestr2);
end
...
fprintf(fid,'%s\n',geostr2);
[numlvs,numrows,numcols]=size(LVTS.values);
...
datastr1=strcat('LVT array dimensions nrows=',num2str(numrows),...
...
igo=1;
clear DisplayCONUS_LVT; % This step is needed to clear persistent variables
while igo>0 % This setup up a loop to processing various file until user decides to STOP RUN
    ...
    if(a1==1)
        igo=0;
        clear DisplayCONUS_LVT;
    else
        igo=1;
        nowlevelstr=char(LevelSets{indL,1});
        ilevel=str2num(nowlevelstr);
        titlestr=strcat('LVT-Level-',num2str(ilevel),'-',filename);
        [LVT1DSF,sgoodfrac]=DisplayCONUS_LVT(titlestr,ilevel);
        titlestr=strcat('LVT-Level-',num2str(ilevel),'-Hist-',filename);
        [ValidLVT]=PlotLVTHistogram(LVT1DSF,sgoodfrac,ilevel,titlestr);
        titlestr=strcat('LVT-Level-',num2str(ilevel),'-CumilDist-',filename);
        PlotLVTCumilDist(ValidLVT,sgoodfrac,ilevel,titlestr);
    end
end
add(rpt,chapter);
fprintf(fid,'%s\n','-----Finished LVT Data----');
disp('-----Finished Processing LVT Data-----');
end
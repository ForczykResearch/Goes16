function PlotCloudTopHeightsCumilDist(HT,titlestr)
% This routine will plot the cumilative distribution of cloud top heights
% in units of Km
% Written By: Stephen Forczyk
% Created: Nov 24,2020
% Revised: Jan 4,2021 added code to create pdf report
...
fprintf(fid,'%s\n','-----Create Cloud Cumilative Height Distirubution Plot-----');
...
maxHT=max(HTSort);
HTCumil=zeros(numvals,1);
for n=1:numvals
   HTCumil(n,1)=HTSort(n,1); 
end
...
ypi=zeros(101,1);
ypi(1)=0;
ypi(101)=HTCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=HTCumil(ptileind,1);
end
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
ptile75ht=ypi(76,1);
ptile100ht=ypi(101,1);
htstr1=strcat('Cloud Height  25 Ptile Height=',num2str(ptile25ht,6),'-Km');
htstr2=strcat('Cloud Height  50 Ptile Height=',num2str(ptile50ht,6),'-Km');
htstr3=strcat('Cloud Height  75 Ptile Height=',num2str(ptile75ht,6),'-Km');
htstr4=strcat('Cloud Height 100 Ptile Height=',num2str(ptile100ht,6),'-Km');
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
ylabel('Cloud Top Height-Km','FontWeight','bold');
...
tx2=.18;
ty2=.06;
txtstr2=strcat('Cloud HT 25ptile=',num2str(ptile25ht,6),'-HT 50 ptile=',num2str(ptile50ht,6),...
    '-HT 75 ptile=',num2str(ptile75ht,6),'-HT 100 ptile=',num2str(ptile100ht,6),'-all HT in Km');
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    add(chapter,Section('Cumil Distribution Of Cloud Top Heights'));
    ...
    image.ScaleToFit=0;
    add(chapter,image);
    parastr1='This chart provides a Cumilative plot of the Cloud Top Heights in Km.';
    parastr2=' The first line below the plot provides details of when the scan was taken.';
    parastr3=' The last line provides a few basic statistics on cloud top height distribution.';
    parastr4=strcat(' For example the 25% ptile cloud height was=',num2str(ptile25ht,6),'-km,',...
        ' while the max height was=',num2str(ptile100ht,6),'-km.');
...
disp(dispstr);
fprintf(fid,'%s\n','-----Finished Cloud Cumilative Height Distirubution Plot-----');
end

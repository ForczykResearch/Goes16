function [ValidSI]=PlotSIStabilityIndexHistogram(SI1DSF,sgoodfrac,titlestr)
% This routine will plot a histogram of the SI Stability Index
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Showalter Stability Index Histogram Plot-----');
...
numpts=length(SI1DSF);
[istable]=find(SI1DSF>=-3);
numstable=length(istable);
[iunstable]=find(SI1DSF<-3);
numunstable=length(iunstable);
fracunstable=numunstable/(numpts);
[ivalid]=find(SI1DSF>=-10 & SI1DSF<=4);
numvalid=length(ivalid);
ValidSI=zeros(numvalid,1);
index=0;
for kk=1:numpts
    if((SI1DSF(kk,1)>=-10) && (SI1DSF(kk,1)<=4))
        index=index+1;
        ValidSI(index,1)=SI1DSF(kk,1);
    end
end
...
num100=numvalid;
minValidSI=min(ValidSI);
maxValidSI=max(ValidSI);
val25=ValidSI(num25,1);
val50=ValidSI(num50,1);
val75=ValidSI(num75,1);
val100=ValidSI(num100,1);
statstr1=strcat('25th ptile SI=',num2str(val25,6),'-50 ptile SI=',num2str(val50,6));
statstr2=strcat('75th ptile SI=',num2str(val75,6),'-100 ptile SI=',num2str(val100,6),'-all in Deg-C.');
fprintf(fid,'%s\n',statstr1);
fprintf(fid,'%s\n',statstr2);
% Now plot the histogram of the SI Distribution
movie_figure1=figure('position',[hor1 vert1 widd lend]);
set(gcf,'MenuBar','none');
set(gca,'Position',[.16 .18  .70 .70]);
edges=linspace(-10,4,31);
h=histogram(ValidSI,edges,'Normalization','probability','FaceColor','b');
xlabel('SI Stability Index-Deg-C','FontWeight','bold');
...
tx2=.18;
ty2=.06;
txtstr2=strcat('Min SI Index=',num2str(minValidSI,4),'-Max ValidSI=',num2str(maxValidSI,4));
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    pdftxtstr=strcat('SI Stability Index For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr122='This chart shows this distribution of SI Stability metric as a histogram.';
    if(fracunstable<.05)
        parastr123=strcat('The fraction of unstable pixels-',num2str(fracunstable,5),'-is very low so the scene is quite stable.');
    elseif(fracunstable>=.05)
        parastr123=strcat('The fraction of unstable pixels-',num2str(fracunstable,5),'-is over 5% so there are significant regions of instability.');         
    end
...
%    add(rpt,chapter);
    ...
    disp(dispstr);
    fprintf(fid,'%s\n','-----Finished Showalter Stability Index Histogram Plot-----');
    fprintf(fid,'\n');
end

...
% Created: Jan4,2021
% Revised: Jan 6,2020 break point cleanup
...
global CTopTemp1S Algo2S ProcessParamTS AlgoProductTS Error1S CloudPixelsS;
global idebug;
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','**************Start reading Full Disk Cloud Top Temp data***************');
...
CTopTempS=struct('values',[],'long_name',[],'standard_name',[],'unsigned',[],'valid_range',[],...
    'scale_factor',[],'add_offset',[],'units',[],'resolution',[],'coordinates',[],...
    'grid_mapping',[],'cell_methods',[],'ancillary_variables',[],'FillValue',[]);
...
for i = 0:numvars-1
    ...
    for j = 0:numatts - 1
        ...
        if (a10==1)
            ...
            if(a1==1)
                CTopTempS.long_name=attname2;
            end
            a1=strcmp(attname1,'standard_name');
            if(a1==1)
                CTopTempS.standard_name=attname2;
            end
            a1=strcmp(attname1,'_FillValue');
            if(a1==1)
                CTopTempS.FillValue=attname2;
            end
            a1=strcmp(attname1,'units');
            if(a1==1)
                CTopTempS.units=attname2;
            end
            a1=strcmp(attname1,'_Unsigned');
            if(a1==1)
                CTopTempS.unsigned=attname2;
            end
            a1=strcmp(attname1,'valid_range');
            if(a1==1)
                CTopTempS.valid_range=attname2;
            end
            a1=strcmp(attname1,'resolution');
            if(a1==1)
                CTopTempS.resolution=attname2;
            end
            a1=strcmp(attname1,'coordinates');
            if(a1==1)
                CTopTempS.coordinates=attname2;
            end
            a1=strcmp(attname1,'scale_factor');
            if(a1==1)
                CTopTempS.scale_factor=attname2;
            end
            a1=strcmp(attname1,'add_offset');
            if(a1==1)
                CTopTempS.add_offset=attname2;
            end
            a1=strcmp(attname1,'resolution');
            if(a1==1)
                CTopTempS.resolution=attname2;
            ...
            if(a1==1)
                CTopTempS.grid_mapping=attname2;
            end
            a1=strcmp(attname1,'cell_methods');
            if(a1==1)
                CTopTempS.cell_methods=attname2;
            end
            a1=strcmp(attname1,'ancillary_variables');
            if(a1==1)
                CTopTempS.ancillary_variables=attname2;
            end
        elseif (a20==1)
            attname1 = netcdf.inqAttName(ncid,i,j);
            attname2 = netcdf.getAtt(ncid,i,attname1);
...
netcdf.close(ncid);
fprintf(fid,'%s\n','Finished reading Full Disk Cloud Top Temp Data');
...
if(isavefiles==1)
    ...
    actionstr='save';
    varstr1='CTopTempS MetaDataS DQF2S tS yS xS tBS goesImagerS AlgoProductTS';
...
% Get some stats on the cloud Top Tempss
CTopTemp=CTopTempS.values;
[nrows,ncols]=size(CTopTemp);
CTopTemp1D=reshape(CTopTemp,nrows*ncols,1);
minCTopTemp=min(CTopTemp1D);
maxCTopTemp=max(CTopTemp1D);
a1=isnan(CTopTemp1D);
numnans=sum(a1);
numtotpix=nrows*ncols;
fracNaN=numnans/numtotpix;
goodfrac=1-fracNaN;
statstr1=strcat('Min Cloud Top Temp=',num2str(minCTopTemp,6),'-Max Temp=',num2str(maxCTopTemp,6));
fprintf(fid,'%s\n',statstr1);
statstr2=strcat('Frac Of Pixels Returning Valid Temp=',num2str(goodfrac,6),'-Frac That are NaN=',num2str(fracNaN,6));
fprintf(fid,'%s\n',statstr2);
% Display the Cloud Top Temp Data on the Earth Disk
...
DisplayCloudTopTempsRev1(titlestr)
% Display a Histogram of the CloudTop Temp distribution
titlestr=strcat('CloudTopTempHist-',filename);
PlotCloudTopTempHistogram(titlestr)
% Display the Cumilative Cloud Top Temperature Distribution
titlestr=strcat('CloudTopTempCumilDist-',filename);
PlotCloudTopTempCumilDist(titlestr)
end

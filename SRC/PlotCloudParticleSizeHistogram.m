function PlotCloudParticleSizeHistogram(OptPS1DSF,sgoodfrac,titlestr)
% This routine will plot a histogram of the Cloud Particle Size
% Written By: Stephen Forczyk
% Created: Feb 1,2021
% Revised: Mar 31,2021 renamed to Cloud Particle size
% changes
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create Cloud Particle Size Histogram Plot-----');
...
fprintf(fid,'%s\n',statstr1);
fprintf(fid,'%s\n',statstr2);
% Now plot the histogram of the Cloud Particle Size Distribution
...
xlabel('Cloud Particle Size-microns','FontWeight','bold');
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('Cloud Particle Size Histogram'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('Cloud Particle Size For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr62='This chart shows this distribution of cloud particle sizes as a histogram.';
    ...
    disp(dispstr);
    fprintf(fid,'%s\n','-----Finished Cloud Particle Size Histogram Plot-----');
    fprintf(fid,'\n');
end

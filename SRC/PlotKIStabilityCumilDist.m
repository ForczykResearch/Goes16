function PlotKIStabilityCumilDist(ValidKI,sgoodfrac,titlestr)
% This routine will plot the cumilative distribution of the K Index
% estimates
% Written By: Stephen Forczyk
% Created: Feb 21,2021
% Revised: Feb 22,2021 added logix to create Stability matrix
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create KI Stability Index Cumilative Distribution Plot-----');
...
numvals=length(ValidKI);
val01=ValidKI(round(.01*numvals),1);
val50=ValidKI(round(.50*numvals),1);
val99=ValidKI(round(.99*numvals),1);
KICumil=ValidKI;
% Create a stability array
KI=KIS.values;
KI=KI';
[nrows,ncols]=size(KI);
KIStability=zeros(nrows,ncols);
numnan=0;
for ii=1:nrows
    for jj=1:ncols
        val=KI(ii,jj);
...
htstr1=strcat('KI 25 Ptile=',num2str(ptile25ht,6),'-Deg-C');
htstr2=strcat('KI 50 Ptile=',num2str(ptile50ht,6),'-Deg-C');
htstr3=strcat('KI 75 Ptile=',num2str(ptile75ht,6),'-Deg-C');
htstr4=strcat('KI 100 Ptile=',num2str(ptile100ht,6),'-Deg-C');
htstr5=strcat('The fraction of unstable KI values is=',num2str(fracunstable,6));
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('KI 25ptile=',num2str(ptile25ht,6),'-KI 50ptile=',num2str(ptile50ht,6),...
    '-KI 75ptile=',num2str(ptile75ht,6),'-KI 100 ptile=',num2str(ptile100ht,6),'-all in Deg-C',...
    '-fraction of unstable pixels=',num2str(fracunstable,6));
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',8);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('KICumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    pdftxtstr=strcat('KI Index Data For File-',GOESShortFileName);
...
% Now add some text 
    br = PageBreak();
    add(chapter,br);
    parastr171=strcat('The KI Stability Index Data for this chart was taken from file-',GOESShortFileName,'.');
    parastr172='This chart shows the cumilative distribution of all valid KI Stability Index estimates.';
    parastr173=strcat('The median value for the KI is-',num2str(ptile50ht,6),'.');
    parastr174=strcat('The red box shows the unstable region.','The fraction of pixels reporting in the unstable region is=',num2str(fracunstable,6),'.');
    if(ptile95ht<=30)
        parastr175=strcat('Since the 95th percentile value-',num2str(ptile95ht,5),'-is less than a value of 30 the current scene is quite stable.');
    else
        parastr175=strcat('Since the 95th percentile value-',num2str(ptile95ht,5),'-is greater than a value of 30 the current scene shows significant instability.');
    end
    parastr179=strcat(parastr171,parastr172,parastr173,parastr174,parastr175);
...
%    add(rpt,chapter);
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished KI Stability Index Cumilative Distribution Plot-----');
    fprintf(fid,'\n');
end

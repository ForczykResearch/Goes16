function  DisplaySmokeMask(titlestr)
% Display the smoke mask from the GOES16/17 data
...
fprintf(fid,'%s\n',smokestr1);
smokemin=min(min(Smoke));
smokemax=max(max(Smoke));
smokestr2=strcat('Smoke mask min value=',num2str(smokemin,3),'-mask max value=',num2str(smokemax,3));
fprintf(fid,'%s\n',smokestr2);

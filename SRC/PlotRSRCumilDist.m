function PlotRSRCumilDist(ValidRSR,titlestr)
% This routine will plot the cumilative distribution of the RSR
% or Reflected Solar Radiation metric
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Create RSR Cumilative Distribution Plot-----');
...
numvals=length(ValidRSR);
val01=ValidRSR(round(.01*numvals),1);
val50=ValidRSR(round(.50*numvals),1);
val99=ValidRSR(round(.99*numvals),1);
...
ypi(101)=RSRCumil(numvals,1);
for n=2:100
    ptile=(n-1)/100;
    ptileind=round(ptile*numvals);
    ypi(n,1)=RSRCumil(ptileind,1);
end
ptile25ht=ypi(26,1);
ptile50ht=ypi(51,1);
ptile75ht=ypi(76,1);
ptile100ht=ypi(101,1);
htstr1=strcat('RSR 25 Ptile=',num2str(ptile25ht,6));
htstr2=strcat('RSR 50 Ptile=',num2str(ptile50ht,6));
htstr3=strcat('RSR 75 Ptile=',num2str(ptile75ht,6));
htstr4=strcat('RSR 100 Ptile=',num2str(ptile100ht,6));
fprintf(fid,'%s\n',htstr1);
fprintf(fid,'%s\n',htstr2);
fprintf(fid,'%s\n',htstr3);
fprintf(fid,'%s\n',htstr4);
...
tx2=.18;
ty2=.06;
txtstr2=strcat('RSR 25ptile=',num2str(ptile25ht,6),'-RSR 50ptile=',num2str(ptile50ht,6),...
    '-RSR 75ptile=',num2str(ptile75ht,6),'-RSR 100 ptile=',num2str(ptile100ht,6),'-all in W/m^2');
txt2=text(tx2,ty2,txtstr2,'FontWeight','bold','FontSize',8);
set(newaxesh,'Visible','Off');
pause(chart_time);
% Save this chart
figstr=strcat('RSRCumilDist-','Y',num2str(YearS),'D',...
...
if((iCreatePDFReport==1) && (RptGenPresent==1))
    ...
    br = PageBreak();
    add(chapter,br);
    add(chapter,Section('RSR Cumil Distribution'));
    imdata = imread(figstr);
    [nhigh,nwid,ndepth]=size(imdata);
    image = mlreportgen.report.FormalImage();
    image.Image = which(figstr);
    pdftxtstr=strcat('RSR Data For File-',GOESShortFileName);
...
% Now add some text 
    ...
    parastr82='This chart shows the cumilative distribution of all valid RSR estimates.';
    parastr83=strcat('The median value for the RSR is-',num2str(ptile50ht,6),'-in W/m^2.');
    ...
    fprintf(fid,'%s\n',savestr1);
    fprintf(fid,'%s\n','-----Finished RSR Cumilative Distribution Plot-----');
    fprintf(fid,'\n');
end

function Fixedpathdefinition_gui()
% created to allow user to set paths for the GOES16 data reader
% Written By: Stephen Forczyk
% Created: April 3,2021
% Revised: April 9,2021 added more paths
% 
...
a1=exist(mappath,'dir');
a2=exist(matlabpath,'dir');
a3=exist(USshapefilepath,'dir');
a4=exist(dtedpath,'dir');
a5=exist(countyshapepath,'dir');
...
% a17=exist(htmlpath,'dir');
  a18=exist(countyshapepath,'dir');
  a19=exist(nationalshapepath,'dir');
% a20=exist(librarypath,'dir');
  a21=exist(northamericalakespath,'dir');
  a22=exist(govjpegpath,'dir');
  a23=exist(logpath,'dir');
% a24=exist(thrmvurpath,'dir');
% a25=exist(outputpath,'dir');
% if(a1==0)
%     dispstr=strcat('Sorry but directory-',mappath,'-does not exist on this machine');
%     UpdateMessageBox(dispstr,handles);
% elseif(a1==7)
%     dispstr=strcat('Ok directory-',mappath,'-verified on this machine');
%     UpdateMessageBox(dispstr,handles);
% end 
% if(a2==0)
%     dispstr=strcat('Sorry but directory-',matlabpath,'-does not exist on this machine');
%     UpdateMessageBox(dispstr,handles);
% elseif(a2==7)
%     dispstr=strcat('Ok directory-',matlabpath,'-verified on this machine');
%     UpdateMessageBox(dispstr,handles);
% end 
% if(a3==0)
%     dispstr=strcat('Sorry but directory-',USshapefilepath,'-does not exist on this machine');
%     UpdateMessageBox(dispstr,handles);
% elseif(a3==7)
%     dispstr=strcat('Ok directory-',USshapefilepath,'-verified on this machine');
%     UpdateMessageBox(dispstr,handles);
% end
% if(a4==0)
...
htext0  = uicontrol('Style','text','String','Specify Fixed Paths',...
           'Position',[420,830,250,30],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',14,'FontWeight','bold');
%---------------- Show GOES 16 Reader Fixed Path Info-------------------------
htext1  = uicontrol('Style','text','String','Matlab Maps',...
           'Position',[50,790,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder has Matlab geographic boundary file');
if(a1==7)
    hedit1  = uicontrol('Style','edit','String',mappath,...
...
%---------------- Show Matlab  Path Info-------------------------
htext2  = uicontrol('Style','text','String','Misc Matlab Files',...
           'Position',[50,760,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is where misc Matlab data is stored');
if(a2==7)
   hedit2  = uicontrol('Style','edit','String',matlabpath,...
...
%---------------- Show USStateShapeFiles Path Info-------------------------
htext3  = uicontrol('Style','text','String','USStateShapeFiles',...
           'Position',[50,730,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is where the US state shapefiles are stored');
if(a3==7)
    hedit3  = uicontrol('Style','edit','String',USshapefilepath,...
...
%---------------- Show DTED Path Info-------------------------
htext4  = uicontrol('Style','text','String','DTED Files',...
           'Position',[50,700,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder has the DTED files sorted by longitude');
if(a4==7)
    hedit4  = uicontrol('Style','edit','String',dtedpath,...
...
htext18  = uicontrol('Style','text','String','Country Shape Files',...
           'Position',[50,670,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is where Country Level Shapefiles are stored');
if(a18==7)
    hedit18  = uicontrol('Style','edit','String',countyshapepath,...
...
%---------------- Show NationalShapeFiles Path Info-------------------------
htext19  = uicontrol('Style','text','String','National Shape Files',...
           'Position',[50,640,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder are additional National Level Shape Files');
if(a19==7)
    hedit19  = uicontrol('Style','edit','String',nationalshapepath,...
...
%---------------- Show North American Lakes Path Info-------------------------
htext21  = uicontrol('Style','text','String','North American Lake Files',...
           'Position',[50,610,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is has the North American Geographic Data');
if(a21==7)
    hedit21  = uicontrol('Style','edit','String',northamericalakespath,...
...
%---------------- Show GOVJpeg Path Info-------------------------
htext22  = uicontrol('Style','text','String','GOV Jpeg Files',...
           'Position',[50,580,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is where gov supplied jpegs are stored');
if(a22==7)
    hedit22  = uicontrol('Style','edit','String',govjpegpath,...
...
%---------------- Show Log Path Info-------------------------
htext23  = uicontrol('Style','text','String','Log Files',...
           'Position',[50,550,150,20],'BackGroundColor',[0 0 1],'ForeGroundColor',[1 0 0],...
           'FontSize',12,'FontWeight','bold','TooltipString','This folder is where run log files are stored');
if(a23==7)
    hedit23  = uicontrol('Style','edit','String',logpath,...
...
f.Visible = 'on';
  ...
  function button1_Callback(source,eventdata)
        folder_name = uigetdir(mappath,'Select Matlab Map Path');
        mappath=strcat(folder_name,'\');
        set(hedit1,'String',mappath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected Matlab Map Path as-',mappath);
        disp(dispstr)
        eval(['cd ' mappath(1:length(mappath)-1)]);
        [islash]=strfind(mappath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=mappath(is:ie);
        set(hbutton1,'BackGroundColor',[0 0 1]);
  end
  function button2_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Matlab Data File Path');
        matlabpath=strcat(folder_name,'\');
        set(hedit2,'String',matlabpath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Matlab Data File path as-',matlabpath);
        disp(dispstr)
        [islash]=strfind(matlabpath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=matlabpath(is:ie);
        set(hbutton2,'BackGroundColor',[0 0 1]);
  end
  function button3_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select US States ShapeFiles File Path');
        USshapefilepath=strcat(folder_name,'\');
        set(hedit3,'String',USshapefilepath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected US States Shape File path as-',USshapefilepath);
        disp(dispstr)
        [islash]=strfind(USshapefilepath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=USshapefilepath(is:ie);
        set(hbutton3,'BackGroundColor',[0 0 1]);
  end
  function button4_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path to DTED Files');
        dtedpath=strcat(folder_name,'\');
        set(hedit4,'String',dtedpath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to DTED data as-',sumrytrjpath);
        disp(dispstr)
        [islash]=strfind(dtedpath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=dtedpath(is:ie);
  ...
  function button18_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path to Country Shape Files');
        countyshapepath=strcat(folder_name,'\');
        set(hedit18,'String',countyshapepath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to Country Shape Files as-',countyshapepath);
        disp(dispstr)
        [islash]=strfind(countyshapepath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=countyshapepath(is:ie);
        set(hbutton18,'BackGroundColor',[0 0 1]);
  end
  function button19_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path To National ShapeFile Data');
        nationalshapepath=strcat(folder_name,'\');
        set(hedit19,'String',nationalshapepath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to National ShapeFiles-',nationalshapepath);
        disp(dispstr)
        [islash]=strfind(nationalshapepath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=nationalshapepath(is:ie);
 ...
 function button21_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path To North American Lakes Data');
        northamericalakespath=strcat(folder_name,'\');
        set(hedit21,'String',northamericalakespath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to North American Lakes  Files-',northamericalakespath);
        disp(dispstr)
        [islash]=strfind(northamericalakespath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=northamericalakespath(is:ie);
        set(hbutton21,'BackGroundColor',[0 0 1]);
 end
 function button22_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path To Load Gov Jpeg Data');
        govjpegpath=strcat(folder_name,'\');
        set(hedit22,'String',govjpegpath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to Gove Jpeg Files-',govjpegpath);
        disp(dispstr);
        [islash]=strfind(govjpegpath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=govjpegpath(is:ie);
        set(hbutton22,'BackGroundColor',[0 0 1]);
 end
 function button23_Callback(source,eventdata)
        folder_name = uigetdir(pwdnow,'Select Path To Store Log file Data');
        logpath=strcat(folder_name,'\');
        set(hedit23,'String',logpath);
        ichanged1=ichanged1+1;
        dispstr=strcat('Selected path to Run Log Files-',logpath);
        disp(dispstr);
        [islash]=strfind(logpath,'\');
        numslash=length(islash);
        is=1;
        ie=islash(numslash-1)-1;
        pwdnow=logpath(is:ie);
...
%    set(handles.Set_Paths_Tag,'Checked','On');
    ...
    if(ichanged1>0)
        SaveGoes16ReaderFixedPreferenceFile()
        dispstr=strcat('Saved GOES16 Reader Preferences-',num2str(ichanged1),'-paths were changed');
        disp(dispstr)
    else
  ...
  function nextprefbutton_Callback(source,eventdata)
    ...
    if(ichanged1>0)
        eval(['cd ' matlabpath(1:length(matlabpath))]);
        SaveGoes16ReaderFixedPreferenceFile(handles)
    end
    close(gcf);

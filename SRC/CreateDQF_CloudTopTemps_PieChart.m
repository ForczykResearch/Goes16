function  [bigreasonval]=CreateDQF_CloudTopTemps_PieChart(titlestr)
% This function will show selected DQF struct for cloud top temps
% for a single level
...
fprintf(fid,'\n');
fprintf(fid,'%s\n','-----Start plot routine CreateDQF_Cloud Top Temps_PieChart-----');
...
titlestr2=strcat('DQF-Cloud-Top-Temps-Invalid-Pixels',prefix,suffix);
ht=title(titlestr2);
numvals=length(ph);
cb=jet;
...
tx1=.13;
ty1=.26;
txtstr1='Note that the pie chart is concerned with showing the reason why invalid pixels were declared invalid';
...
eval(cmdString);
plotstr1=strcat('Saved Cloud Top Temps DQF Plot as-',figstr);
disp(plotstr1);
fprintf(fid,'%s\n',plotstr1);
pause(chart_time);
if((iCreatePDFReport==1) && (RptGenPresent==1))
...
close('all');
fprintf(fid,'%s\n','-----Finish plot routine CreateDQF_Cloud Top Temps_PieChart-----');
fprintf(fid,'\n');
end

function DisplayABIL2SuperDetailedFireData(FireLats,FireLons,FireHotValues,itype,fireState,fireStateFP,titlestr)
...
% The scale is this map is roughly fixed at 1 deg by 1 deg containing
% the selected fire
...
h1=axesm('mercator','Frame','on','Grid','on','MapLatLimit',[southEdge1 northEdge1],...
    'MapLonLimit',[westEdge1 eastEdge1],'meridianlabel','on','parallellabel','on','plabellocation',.2,'mlabellocation',.2,...
    'MLabelParallel','south','MLineLocation',.2,'PLineLocation',.2,'MLabelRound',2,'PLabelRound',2,'LabelUnits','dms',...
    'LabelFormat','signed');
set(gcf,'Position',[hor1 vert1 widd lend])
set(gcf,'MenuBar','none');
%set(h1,'XTick',[-122.0 -121.8 -121.6 -121.4 -121.2 -121.0]);
xtickformat('%.2f')
ytickformat('%.2f');
...
HoldFrac=zeros(numrecords,1);
HoldFlag=zeros(numrecords,1);
for n=1:numrecords
    ...
    if(STATEFP==fireStateFP)
        ...
        A1=isnan(CountyLat);
        numnan1=sum(A1);
        [xin,yin,Ind]=Inside(XB,YB,CountyLon,CountyLat);
        numin1=length(xin);
        numtot1=length(CountyLon);
...
%         end
        if(goodfrac1>0)
            for nn=1:numin1
                indx=Ind(1,nn);
                nowLat=CountyLat(1,nn);
                nowLon=CountyLon(1,nn);
                plotm(nowLat,nowLon,nowColor);
            end
        end
    else
        imiss=imiss+1;
